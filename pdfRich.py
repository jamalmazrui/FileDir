r"""pdfRich.py -- convert a PDF to RICH Markdown, for the Homer Tools.
...
tables -- and writes Markdown. FileDir then reads that Markdown, or hands it to Pandoc for
any other target, so one rich conversion serves every one of them.

Nothing here needs Microsoft Word. Install the requirement once with
installPdfTools.cmd in the FileDir folder.

A detailed log is written beside the target as <target>.log whatever
happens, so FileDir can quote the reason.
"""
...
def main():
    ...
    try:
        ...
        say("Run installPdfTools.cmd in the FileDir program folder to install it.")
        say("It needs Python, which the FileDir installer offers as a checkbox.")
        writeLog(pathTarget)
    ...
    # On success the log is not kept: FileDir reads it only to explain a
