...
**The rule is now checked at build time.** An accessible name that repeats a
caption or a label is easy to add again, because adding one looks like an
accessibility improvement. The audit fails the build on any control in
Dialogs.cs, Lbc.cs or MediaPlayer.cs that sets one, so the forty-eight cannot
quietly come back.

The reasoning behind it is written down in the developer notes, as four rules:
...
sayTutorial takes the name of a script, so a list of tasks becomes a set of
short tutorials: sayTutorial Tutorial_Tagging speaks Tutorial_Tagging.inix into
Tutorial_Tagging.mp3. makeTutorial writes every Tutorial*.inix in the folder
into Tutorials.md in one pass, numbered 0, 0a, 0b and so on, with the contents
lines taken from the headings it just wrote. Adding a tutorial means adding a
file and nothing else.

The speech runs at four for the narrator and six for the screen reader's
stand-in, on the scale System.Speech uses, where zero is the pace of somebody
reading aloud to a stranger. The people this is for listen to synthetic speech
all day, and the reader's own voice is always the faster of the two. The gaps
between pieces were shortened as well, and the opening sentence cut to what it
has to say: this is a simulation, one voice narrates, the other stands in for
the screen reader. Silence and preamble are listening time too.

Tutorial_Tagging.inix is the first task tutorial: tag two files, copy them
somewhere, four steps.

**And sayTutorial.ps1 had no trap, which the audit caught and the build
reported.** Every delivered script must put the reason it stopped into its own
log, because a console message with an empty log is nothing to debug from. It
has one now.

**A walkthrough, written from what a screen reader actually says.** The feature
set has grown faster than anybody's patience for learning it, so there is now a
first walk: twelve steps from opening a folder to closing the Player, in
Tutorials.md as section 0.

It is generated rather than written. Tutorial.inix holds one step to a section,
and each step says three things -- what a narrator explains, which key to press,
and what JAWS says in answer -- with the spoken lines taken from real speech
histories rather than from anybody's idea of what ought to be said.

makeTutorial.py turns that into the walkthrough section of Tutorials.md, between
two markers, leaving everything outside them exactly as it was, and the build
runs it before the documents are converted. A generated section in an existing
document rather than a new one: a new document would need a line in the
installer, a line in the repository list, and a place in every list of
documents, and the walkthrough is a tutorial, so it belongs with the tutorials.

sayTutorial.cmd speaks the same file into Tutorial.mp3, with a Windows voice for
the narrator and a second one standing in for the screen reader, joined by the
ffmpeg FileDir already ships. It says in its first sentence that it is a
simulation, because it is one. Nothing is fetched and nothing is generated by a
service, so it sounds the same every time -- which is what documentation has to
do.

The format is chosen with one eye on what comes next. NVDA's own system tests
send keys and assert the speech that answers them; the same file could be
replayed against NVDA, or checked against an exported JAWS history, and the
tutorial that teaches somebody would become the test that catches the next
...
which holds the dialogs FileDir had before Lbc: Input, the folder prompt, Pick,
Confirm, the button dialogs, all of them. Every one set AccessibleName to the
words already on the button or on the label beside it, so each control was named
twice over -- which is precisely the repetition that was reported and precisely
the mechanism guessed at.

None of them sets it now. A button carries its own caption, a text box carries
the label before it, and Windows reports both without help.

This does not explain the window TITLE being announced two or three times as a
dialog opens. That is a separate thing, it happens in Lbc dialogs and in these
older ones alike, and nothing in either file asks for it.

**The number in front of each track is gone.** A line beginning with a digit
cannot be reached by pressing its first letter, and first-letter navigation is
how a list of sixty titles is actually used. Where a track sits in the queue is
something a screen reader reports on its own, and Alt+Shift+W says it in words.

**Space plays and pauses.** Scroll Lock was meant to and never arrived; it is a
toggle key, and something between the keyboard and the dialog keeps it. Space is
what mpv's own window uses and what a hand reaches for. It is claimed everywhere
in the dialog except on a button, where Space is how Windows presses the thing
with focus, and except in a box being typed in. The Jump and Filter prompts are
windows of their own, so a search term with a space in it was never in question.
Scroll Lock keeps its binding for anyone whose keyboard delivers it.

**Jump, find and filter belong to the queue, wherever the cursor is.** They
acted on whichever list had focus, so Control+J pressed from Order of list
searched the wrong list. A dialog can now nominate the list its find keys serve,
and the player nominates the queue.

They also leave the keyboard where they found it. A find that drags focus into
the list has decided where the person should be working, and getting back is
their problem. The list's cursor moves and nothing else does -- and when the
list is not where the cursor is, the track landed on is spoken, because a screen
reader only reads a list line when that list has focus.

**The forced access-key underline was taken out again.** It was added on the
theory that a screen reader announces "Alt+E" only while Windows is drawing the
underline; it made no difference to what JAWS said, and it put an extra
window-state message into the moment a dialog opens, which is the moment being
investigated for saying too much. An unproven fix with a known cost is not worth
keeping.

**Play List opens the Homer Player now.** Control+Shift+L used to ask for a
file name, write an .m3u, and hand it to mpv's own window. Writing a play list
is still available and is now where it belongs -- one of the choices in Output
Type, on Shift+O, beside the other things a set of files can become. Playing is
a different job from filing, and asking for a file name before anything can be
heard was a form to fill in on the way to pressing play.

So the tagged files, the play list or media file you are on, and the media links
in the document you are on all open in the player, which drives mpv directly
rather than starting a window and letting go.

**Nothing mpv cannot read is put in front of it.** mpv given something it cannot
play draws an error on its own display, which is never spoken: a screen reader
user gets silence and a window that will not answer. The shared class now knows
what mpv is asked to read -- the audio and video extensions, play lists, and any
web address, since those go to yt-dlp, which knows more than an extension does
-- and anything else is left out of the queue. What was left out is said once, as
a count.

**Output Type offers a play list.** "Play list of these files" appears among the
audio and video choices on Shift+O. It is the one choice there that makes a
single file from many rather than converting each, and it leaves out anything
mpv cannot read, because a play list that stops at a spreadsheet is not a play
list.

**Alt+Shift+M in the player writes track notes.** A Markdown file of what is
showing, in the order shown: the sort chosen in Order of list, and only the
tracks a filter has left. One heading per track, and under it only the fields
that are known -- a line saying "Presenter: unknown" is a line to listen to for
nothing.

**Hotkeys.ini became Hotkeys.inix.** No Homer tool should own a file with a
.ini extension: .inix is the Homer format, and .ini is what it supersedes. The
file itself is unchanged -- the same command, key and description on each line
-- and everything that reads or ships it follows: makeKeyMap.py, which generates
KeyMap.cs and Hotkeys.md from it, the audit, the build script, the installer
script, the repository file list, the developer notes, and the program itself,
which still reads a copy in its own folder ahead of the compiled defaults.

The old Hotkeys.ini in the project folder is no longer named by anything.
cleanFileDir will report it as not belonging, which is the right answer.

**Play Queue became the Homer Player, on Control+Shift+H.** Q was the wrong
letter and there was never an argument for it beyond the key being free. In
FileDir, Q is the QUICK family: Control+Q opens a quick folder, Shift+Q makes a
quick shortcut, Alt+Shift+Q makes a quick URL. A playback command wearing Q was
borrowing a letter that already means something else, which is the one thing an
Lbc binding must not do.

Every combination of the four candidate letters was checked against the hotkey
table. L is used four times over -- List Files, List, List Tagged, Play List --
and its last free chord is Alt+Control, which is reserved for desktop shortcuts.
P is used five times, from Say Path to Path List to Clipboard. T is used six.
H is nearly empty: only Alt+Shift+H, the Hotkey Summary, is taken.

So Control+Shift+H, and the command is named Homer Player, which is what the
dialog has been called for several versions. H for Homer, and it sits beside
Control+Shift+L for Play List -- the two ways of playing the same things,
sharing a chord family and differing by one letter that says which is which.

**F3 still did nothing, and there were two reasons, both mine.**

The keys were dispatched from a KeyDown handler. DbDo and EdSharp both dispatch
from ProcessCmdKey, and the fault shows why: a key can be taken by the control,
by the form's own dialog-key handling, or by the screen reader before any
KeyDown handler is reached. The jump, find and filter keys now go through
ProcessCmdKey, in a hook of the dialog's own beside the one an application uses,
so nothing can take them first.

And the dialog asked Form.ActiveControl which control had focus. That is not
what ActiveControl means: it gives the active CHILD of the form, which for these
dialogs is the panel holding everything, so asking it for a list box returned
null every time. The chain is walked to the bottom now, and the focused control
found by asking rather than assuming.

Each of those keys also writes a line to the log when it is pressed -- which
term it has, or that the control with focus is not a list -- so a report that
says "nothing happened" can be told apart from one where nothing arrived.

**Alt+Enter shows the properties as text rather than a list.** A list gives
whole lines and nothing else. A read-only memo gives the arrow keys for
character, word and line, Shift with them for selecting, and Control+C for
copying a piece rather than a line -- which is what a person wants from an
address they need half of. It has an OK button and no label, being the one case
where a control needs none: it fills its own window and the title says what it
holds.

**The window titles were made short.** A window's title is read every time the
window is touched, and "Homer Player - links from AmericanScandal.htm" was being
read three times on opening and again on every return to it. The title is now
just "Homer Player"; where the queue came from lives in the Track list's own
label, which is read on entering the list and not before. The properties window
lost the track name from its title for the same reason -- the name is in the
list it is showing.

**F3 was reaching nothing, and the dialog was being read twice.** Two faults
from the same habit of hanging behaviour on a control and hoping the key gets
there.

The find keys were on the list box's own KeyDown, which is behind everything the
dialog and the control may do with a key first. They are claimed by the dialog
now, before any control sees them, and the list-level handler defers to the same
code so there is one set of rules rather than two.

And the dialog set focus twice on opening -- once through ActiveControl, once
again from a Shown handler -- which gives a screen reader two arrivals to
announce and reads the whole dialog out twice. Focus starts in one place now.

**The prompt says Jump, because the key is J.** A prompt headed Find leaves the
letter unexplained; one headed Jump teaches itself every time it opens, and it
is the word FileDir's directory window already uses for the same key. Filter
keeps its own word for the same reason. Naming a prompt after its key is worth
more than naming it after its function.

**Alt+Enter shows everything known about a track**, which is what Alt+Enter
means in Windows and in FileDir's own list. Name, presenter, length and address
first; then, for a file on this computer, everything ExifTool can read out of it
-- artist, album, codecs, bit rate, the date it was recorded. Sorted by field
name and shown as a list rather than a page of text, because forty fields are
searched rather than read: Control+J jumps to a field, Control+F filters, and
Control+C copies the line.

**Finding and filtering, in every Homer list.** A long list is unusable without
them, and a screen reader user needs them more than anyone: reading sixty lines
to find one is the difference between a list and a wall. The keys are the
directory window's own, so one habit serves both.

Control+J finds a substring, case ignored, from where the cursor is;
Control+Shift+J searches backwards. F3 and Shift+F3 carry on with the same
substring, and pressing either with nothing to repeat asks rather than refusing.
Control+F shows only the items that match, and Control+Shift+F shows everything
again. A filter that would match nothing is not applied: an empty list is a dead
end, and the count is a real answer on its own.

Both prompts are combo boxes with the last ten answers in them, newest first, no
repeats. Where that history lives is the application's business rather than
Lbc's: FileDir keeps it in its own .inix under a History section, tab separated,
one line per prompt. An application that wires nothing gets a history that lasts
as long as the program runs.

Filtering means a row on screen is no longer the same thing as an item in the
list, so Lbc keeps the full list and answers listSourceIndex; the player asks it
before turning a row into a place in the queue, and rebuilds through
setListItems when a sort changes what is showing.

**Stop was emptying the play list.** mpv's stop command does more than the word
suggests: it stops playback AND CLEARS THE PLAY LIST. So the first Execute
playback worked, Stop wiped the queue, and everything afterwards did nothing at
all while sixty tracks still sat on screen. That is the hang -- not a hang, an
empty player wearing a full list.

Stop is now simply a pause: it stops playing and leaves the place alone, in the
queue and in the track, and Execute playback carries on from there. It is called
Stop rather than Pause because Previous track already holds the P. mpv's own
version is still available in the shared class under a name that says what it
does, clearAndStop.

Two announcements were wrong for the same underlying reason. A track was
announced when it was LOADED rather than when it started playing, so the first
name was read out before anything had happened; and Stop left the player idle,
which the watcher reported as "End of queue". Both now wait for playback to be
real.

**Moving somewhere now plays it.** A person who cannot see the display finds out
where they have landed by listening to it, so Next track, Previous track, the
chapter buttons, the jumps, the ends of the track and the timeline all clear the
pause. The commands that only report -- Alt+Shift+A and the rest -- leave it
alone.

**And the fixed dialog frame went back to sizable.** It was tried to stop JAWS
answering "not in a dialog box", and it made no difference; it could not. JAWS
decides that from the window class, and a real dialog is class #32770, which
Windows gives only to windows built from a dialog template. Every WinForms form
is WindowsForms10.Window whatever border it wears, so no Lbc dialog can ever be
a dialog box in that sense -- and none of the ones in DbDo or EdSharp is either.
The commands that matter, reading the controls and the status line, work
regardless, and a long track list would rather be resizable.

**The queue started playing the moment the dialog opened.** mpv begins as soon
as it is given a file, so handing it sixty tracks was enough to start the first
one talking over the window that had just appeared. It is paused before anything
is loaded now, and Execute playback, Enter or Scroll Lock is what starts it --
which is what the button was for.

**The status note was rewritten twice a second**, because the position was in
it. A status bar that changes that often is one a screen reader may decide to
read out, which is the opposite of what a status bar is for here. It now carries
the track, its number, the state and the length -- facts that change when the
person does something -- and the position is left to Alt+Shift+A and the Where
in track slider, which are asked rather than announced.

**And JAWS was answering "not in a dialog box".** An Lbc dialog was built with a
sizable border, which makes it a window rather than a dialog as far as Windows
is concerned, so the screen reader commands that read a dialog had nothing to
read. Every Lbc dialog now uses the fixed dialog frame. Nothing is lost by it:
these dialogs size themselves to their contents and scroll past the cap, so
there was never much to drag.

**The Shift bindings landed outside the handler that reads the key.** When they
replaced the keypad block, they were written above the line that opens the
delegate rather than below it, so twelve statements testing keyData sat in the
method where no keyData exists. csc said so twelve times over, in the plainest
possible terms, and the fix was to move one line.

Worth recording because of what it says about editing by replacement: a block
that reads correctly can still be in the wrong scope, and brace counting will
not notice, because the braces balance either way.

**The transport keys were bound to keys that never arrive, and are now Shift
with the navigation keys.** The first attempt used NumPad0 through NumPad9 on
the reasoning that those are distinct virtual keys with nothing else to do. They
are -- when Num Lock is on. A screen reader turns Num Lock off and leaves it
off, so the keypad sends exactly what the six-pack sends: keypad 4 is Left,
keypad 8 is Up, keypad 5 is Clear. The whole scheme was unreachable on the
machine it was written for.

Shift with the navigation keys replaces it, and works the same on both pads for
the same reason. Bare navigation belongs to whatever control has focus, and
Control with those keys belongs to the dialog, where Control+Home and
Control+End move between fields. Shift with them is free: this dialog has
nothing to tag, and a single-selection list does nothing with Shift.

Each key keeps the sense it already has, applied to the media: arrows step by
the increment, the Page keys move a chapter, Home and End are the ends of the
track, and Control makes it the first or last track or chapter. Shift with
keypad 5 -- which arrives as Shift+Clear, the chord FileDir already uses for Tag
in its own list -- plays and pauses. The digit bindings are kept for a keyboard
with Num Lock on, but they are no longer the scheme.

**F8 and Shift+F8 mark a piece of a track**, which is what they do everywhere
else in Homer Tools: start the selection, complete the selection. Here the ends
are moments rather than lines. Clip to file then writes that piece out with
ffmpeg, streams copied rather than re-encoded, beside the track it came from or
in the folder FileDir is looking at.

**And it goes on the clipboard as a FILE.** Windows has no clipboard format for
a piece of audio that other programs accept: the old CF_WAVE is legacy and
almost nothing pastes it, and for video there is nothing at all. What every
program on Windows 11 does accept is a file, so the clip is written and the file
is put on the clipboard, where it pastes into a folder, a message, or anything
that takes a dropped file. Same destination, a route that works.

**Go became Execute playback, because Gemini owns Alt+G.** Google registered a
global hotkey with a single modifier, which is a choice the rest of the desktop
has to live with: a hotkey registered that way belongs to whoever asked for it
first, and no dialog can take it back from inside its own window. So the button
moved rather than the key. Run collided with Rate, and Start with Stop, which
has no better name than Stop playback.

Every control now has a name of two words or more, so a mnemonic can come from
either of them: Play list, Order of list, Next track, Previous track, Forward in
track, Backward in track, Increment of jump, Rate percent, Volume percent,
Execute playback, Stop playback, Default settings, Help topics.

**Three controls were added.** Chapter more and Chapter less move within a
track that has chapters, on their own row with the Where in track slider, which
moves playback to a percentage through the track.

Their first names were Chapter ahead and Chapter behind, and the pair did not
read as a pair: one took its letter from the second word and the other from the
first, which is two things to remember where there should be one. Same first
word, contrasting second, letter always from the second. More and less describe
quantity rather than direction, which is the price; a matched shape is worth
more here than a perfect word, because the two are pressed one after the other.

That freed a letter and cost one, so Play list became Track list and the
timeline became Where in track -- plainer than the alternatives considered, and
it echoes Alt+Shift+W, which says where you are in words. The timeline is set as
the cursor arrives at it and does not follow playback afterwards: a control that
changed twice a second would be read out over everything else, and that lesson
has been learned here twice already.

**The sliders were lying about their values.** A screen reader announces a track
bar's position as a percentage of its own range, so a rate of 100 on a scale of
25 to 400 was announced as "20", and a volume of 70 out of 130 as "53" -- both
true of the slider and useless to the listener. Lbc gained LbcTrackBar, whose
accessible object reports the value in the words the label promised.

**The status line now carries a standing note** of what is playing: which track
of how many, its name, whether it is playing or paused, and how far in. It is
not a live region and nothing announces it. It sits there to be read with the
screen reader's own key for the status line, beside whatever tip the control
with focus has put there.

**The default button follows the player.** With nothing playing, Enter is Go,
because starting is the only thing left to want; while something plays, Enter is
Stop. Control+Enter is Go throughout, whatever the default happens to be at the
time and wherever the cursor is, so starting something new never depends on
working out which button Enter would press. The Tracks and Order lists handle
Enter themselves and are unaffected. Escape closes and everything chosen is
written on the way out.

**Reading a list was doing things.** Arrowing down Order sorted the queue on
the way past, and the player moved the cursor in Tracks whenever a new track
began. Both are the same fault: for a screen reader user, moving through a list
IS how the choices are read, so a list that acts as it is passed over cannot be
read without being obeyed, and a cursor that moves on its own means a Say Line
somewhere else reads a track name instead.

Neither list has a selection handler now. The cursor moves, the reader reads,
nothing happens. Go applies the order the cursor is on and plays the track the
cursor is on, and Enter in either list does the same. Stop first is not
required: Go on a playing queue simply moves to what was chosen. The player
never touches the cursor again -- a track beginning is announced, and that is
all.

The two sliders keep acting at once, because moving a slider IS changing it,
which is what its arrow keys have meant in Windows since sliders existed.

**Scroll Lock did nothing, and the reason is worth recording.** It is a toggle
key: Windows acts on it, and a KeyDown handler behind KeyPreview may never see
it. It is claimed through ProcessCmdKey now, which runs ahead of the controls
and ahead of that, and the shared Lbc class gained a commandKey hook so any
dialog can claim a key the same way. It writes a line to the log when it fires,
so the next test can tell a key that never arrived from a command that did
nothing.

**The increment is a list of named steps, not a slider.** 15 and 30 seconds, 1,
3, 5, 10, 15 and 30 minutes, and an hour, smallest first, starting at one
minute. A slider announces its position, so a person heard "4" where they needed
to hear "3 minutes".

**Every queue keeps its own settings**, in HomerPlayer.inix, in a section named
after where the queue came from. A lecture can play at 150 percent and music at
100 without either being set twice. **Defaults**, Alt+D, deletes that section
rather than writing the defaults into it, so a queue that never asked for
anything else picks up a better default later.

**And the access keys were invisible, which is why they were silent.** Windows
hides the underline under an access key until somebody presses Alt, and on most
machines that is the default. The underline is not decoration: it is how the
key is published, and a screen reader that cannot see it does not announce
"Alt+G" as the cursor lands on Go. Every Lbc form now clears UISF_HIDEACCEL on
itself as its handle is created, so its access keys are shown -- and announced
-- from the moment it opens. The ampersands were always there; nothing was
listening for them.
