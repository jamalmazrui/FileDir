...
rem ---- version: generated from FileDir_setup.iss (the SINGLE source of truth) ----
rem The version number lives in exactly ONE place: AppVersion in FileDir_setup.iss.
rem This step reads it and generates Version.cs, which defines BuildVersion.Version;
rem FileDir.cs uses that constant instead of hard-coding a number.  Nothing can drift,
...
set "issVer="
for /f tokens^=2^ delims^=^" %%v in ('findstr /r /c:"^#define AppVersion" "FileDir_setup.iss" 2^>nul') do if not defined issVer set "issVer=%%v"
if not defined issVer for /f "tokens=2 delims==" %%v in ('findstr /r /c:"^AppVersion=" "FileDir_setup.iss" 2^>nul') do if not defined issVer set "issVer=%%v"
rem strip any stray spaces (a version number never contains one)
if defined issVer set "issVer=!issVer: =!"
if not defined issVer (
  echo ERROR: could not read AppVersion from FileDir_setup.iss.
  echo ERROR: could not read AppVersion from FileDir_setup.iss. ^>^> "!log!"
  popd ^& exit /b 1
)
> Version.cs echo // Generated by BuildFileDir.cmd from AppVersion in FileDir_setup.iss.  Do not edit; do not commit.
